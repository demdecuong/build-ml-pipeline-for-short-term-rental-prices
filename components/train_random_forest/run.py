...
def get_inference_pipeline(rf_config):
    ...
    features_preprocessor = ColumnTransformer(
        [
            ...
            (
                "ordinal_categ",
                ordinal_categ_preproc,
                rf_config["features"]["ordinal_categ"],
            ),
    ...
    processed_features = list(
        itertools.chain.from_iterable(
            [x[2] for x in features_preprocessor.transformers]
        )
    )
...
def plot_feature_importance(pipe, feat_names):
    ...
    # NLP importance
    nlp_importance = sum(
        pipe["random_forest_model"].feature_importances_[len(feat_names) - 1 :]
    )
